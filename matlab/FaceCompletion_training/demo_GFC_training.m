...
for iter = begin: 1000 % Solver_G.max_iter  %SIDRA added
    Solver_G.iter = iter;
...
%         fprintf('loss_L1 = %i', loss_G); fprintf('  ');
        delta_G{c} = delta_G_;
        fprintf(trainingLossfile, ' loss_L1 = %4.8f', loss_G); %Sidra added
...
%         fprintf('parsing error = %i', Solver_parsing.loss(iter)); fprintf('  '); 
        fprintf(trainingLossfile, ' Parse_Er = %4.8f\n', loss_parsing); %Sidra added
