...
for i = 1:len/2
    ...
    batch_global(max(1,pos(:,2)):max(1,pos(:,2))+parm.masksize-1, max(1,pos(:,1)):max(1,pos(:,1))+parm.masksize-1, :, i) = ...
        active_G(max(1,pos(:,2)):max(1,pos(:,2))+parm.masksize-1, max(1,pos(:,1)):max(1,pos(:,1))+parm.masksize-1, :, i); %copy the occluded region from occluded images to ground truth images
end
